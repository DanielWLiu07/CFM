...
export function createDecoElements(
  ...
  // Scale decoration counts based on height — baseline ~700px (3 rows of cards)
  const heightScale = Math.max(1, totalH / 700);
  function scaled(base: number) { return Math.round(base * heightScale); }

  function pick<T>(arr: T[]): T { return arr[Math.floor(rng() * arr.length)]; }
  function range(min: number, max: number) { return min + rng() * (max - min); }

  // Stratified Y distribution — divides height into N equal bands,
  // places one item per band with jitter so coverage is even
  function stratifiedY(count: number, spread: number): number[] {
    const lo = -halfH * spread, hi = halfH * spread;
    const bandH = (hi - lo) / count;
    return Array.from({ length: count }, (_, i) =>
      lo + bandH * i + rng() * bandH
    );
  }

  // Stratified X within spread range
  function stratifiedX(count: number, spread: number): number[] {
    const lo = -halfW * spread, hi = halfW * spread;
    const bandW = (hi - lo) / count;
    return Array.from({ length: count }, (_, i) =>
      lo + bandW * i + rng() * bandW
    );
  }
  ...
  // Spread factors — push decorations well beyond the card grid
  const spreadX = 2.2;
  const spreadY = 1.8;

  // ── 2. Pixel dots ──
  const dotCount = scaled(isMobile ? 15 : 28);
  const dotColors = ['#fff', '#4af', '#36c', '#26a', '#5bf'];
  const dotYs = stratifiedY(dotCount, spreadY);
  const dotXs = stratifiedX(dotCount, spreadX);
  for (let i = 0; i < dotCount; i++) {
    ...
    `;
    addDeco(el, dotXs[i], dotYs[i], range(-500, -100));
  }

  // ── 3. Wireframe shapes ──
  const shapeCount = scaled(isMobile ? 6 : 10);
  const shapeTypes = ['square', 'circle', 'diamond'] as const;
  const shapeYs = stratifiedY(shapeCount, spreadY);
  const shapeXs = stratifiedX(shapeCount, spreadX);
  for (let i = 0; i < shapeCount; i++) {
    ...
    `;
    addDeco(el, shapeXs[i], shapeYs[i], range(-350, -60));
  }

  // ── 4. Data/code fragments ──
  const fragments = ['0x4F2A', '>>_', '10110', 'SYS.OK', 'CFM://', 'MEM.64K', 'LOAD *', 'RUN >', '00FF', 'ACK', '0xDEAD', 'NOP', 'PING', 'EOF'];
  const fragCount = scaled(isMobile ? 6 : 10);
  const fragYs = stratifiedY(fragCount, spreadY);
  const fragXs = stratifiedX(fragCount, spreadX);
  for (let i = 0; i < fragCount; i++) {
    ...
    `;
    addDeco(el, fragXs[i], fragYs[i], range(-350, -100));
  }

  // ── 5. Floating scan lines ──
  const lineCount = scaled(isMobile ? 3 : 5);
  const lineYs = stratifiedY(lineCount, spreadY);
  for (let i = 0; i < lineCount; i++) {
    ...
    `;
    addDeco(el, 0, lineYs[i], range(-150, -40));
  }

  // ── 6. Glowing orbs ──
  const orbCount = scaled(isMobile ? 2 : 4);
  const orbYs = stratifiedY(orbCount, spreadY);
  const orbXs = stratifiedX(orbCount, spreadX);
  for (let i = 0; i < orbCount; i++) {
    ...
    `;
    addDeco(el, orbXs[i], orbYs[i], range(-500, -200));
  }

  // ── 7. Cross/plus markers ──
  const crossCount = scaled(isMobile ? 4 : 7);
  const crossYs = stratifiedY(crossCount, spreadY);
  const crossXs = stratifiedX(crossCount, spreadX);
  for (let i = 0; i < crossCount; i++) {
    ...
    el.appendChild(hBar);
    el.appendChild(vBar);
    addDeco(el, crossXs[i], crossYs[i], range(-300, -100));
  }

  // ── 8. Connection lines ──
  const connCount = scaled(isMobile ? 3 : 5);
  const connYs = stratifiedY(connCount, spreadY);
  const connXs = stratifiedX(connCount, spreadX);
  for (let i = 0; i < connCount; i++) {
    ...
    addDeco(el, connXs[i], connYs[i], range(-350, -100), 0, rotZ);
  }
